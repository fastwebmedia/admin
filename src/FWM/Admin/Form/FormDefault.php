...
use AdminTemplate;
use FWM\Admin\Menu\MenuItem;
use Illuminate\Contracts\Support\Renderable;
use Illuminate\View\View;
use Input;
use FWM\Admin\Admin;
use FWM\Admin\Interfaces\DisplayInterface;
use FWM\Admin\Interfaces\FormInterface;
use FWM\Admin\Interfaces\FormItemInterface;
use FWM\Admin\Model\ModelConfiguration;
...
class FormDefault implements Renderable, DisplayInterface, FormInterface
{
	...
	/**
	 * Add a Form Field instance to items
	 * @param null $item
     */
	public function addItem($item = null)
	{
		if ( ! is_null($item) && ($item instanceof FormItemInterface) )
		{
			$this->items[] = $item;
	...
	public function save($model)
	{
		if ($this->model() != $model)
		{
			flash()->error(trans('admin::lang.alert.update.error'));
		...
		array_walk_recursive($items, function ($item)
		{
			if( method_exists($item, 'getIgnored') && $item->getIgnored() ){
				return false;
			}
		...
		flash()->success(trans('admin::lang.alert.update.success', [
			'route' => $this->model()->displayUrl()
		]));
	}
	...
	public function validate($model)
	{
		...
		if ($validator->fails())
		{
			flash()->error(trans('admin::lang.alert.update.validation'));
			return $validator;
		}
		return null;
	}
	...
	public function render()
	{
		$params = [
            ...
            'saveUrl'  => session('_saveUrl', URL::current()),
		];
